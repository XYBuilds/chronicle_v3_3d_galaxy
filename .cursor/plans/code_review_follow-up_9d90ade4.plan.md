...
overview: 在继续 [TMDB Galaxy Dev Plan](.cursor/plans/tmdb_galaxy_dev_plan_5ad6bea5.plan.md) 的 Phase 4 之前，按 [Phase 3.7.0 项目状态与代码审查 实施报告.md](docs/reports/Phase%203.7.0%20项目状态与代码审查%20实施报告.md) 优先完成全量数据验证、根工作区 DX、相机边界与 ESLint 清理。**Bloom**：后续开发阶段保持 `scene.ts` 当前默认值不变；**生产向 Bloom 数值本阶段不定**，留待未来专门迭代（仍可用 `window.__bloom` 本地试验）。色板复杂度作为非阻塞 backlog 记录。
todos:
...
- 主计划 [tmdb_galaxy_dev_plan_5ad6bea5.plan.md](.cursor/plans/tmdb_galaxy_dev_plan_5ad6bea5.plan.md) 中 **Phase 0–3.6 已完成**；**Phase 3.7**（本 follow-up：代码审查与跟进）的主要交付已完成，**camera clamp 仍为 pending**。**Phase 4（Raycaster / Tooltip / Drawer 等）仍为 pending**。
- [Phase 3.7.0 项目状态与代码审查 实施报告.md](docs/reports/Phase%203.7.0%20项目状态与代码审查%20实施报告.md) 的结论与主计划一致：在进入 Phase 4 前，应先消除「仅 subsample 规模验证」带来的性能与视觉盲区。

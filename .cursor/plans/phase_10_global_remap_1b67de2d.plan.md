...
overview: Phase 10 全局视觉重映射：rating→L（P10.1）、距离衰减（P10.2）已落地；P10.3 Bloom「默认开」已**产品收尾为默认关、未来不采用 Bloom**（见 `docs/reports/Phase 10.3 …报告.md`）。P10.4 中与 Bloom-on 文档同步相关项取消。**P10.5**：`视觉参数总表` / Tech Spec §1.2 / Design Spec / Phase 8 P10.0 文案已与实现对齐（completed）。
todos:
  - id: p100-baseline
...
## P10.5 项目文档回填（**completed** — todo `p105-doc-backfill`）

...
- **Camera XY/Z clamp**：`camera.ts` 已接收 `zRange`/`xyRange`，但 wheel/drag 未做 clamp，用户可漫游至数据云外。→ 见 follow-up plan §3.3。

### 非阻塞 Backlog

- **Genre OKLCH 色板可读性**：全量流派多时色差可能不足；留待 Phase 4 后的视觉打磨迭代。→ 见 follow-up plan §5。

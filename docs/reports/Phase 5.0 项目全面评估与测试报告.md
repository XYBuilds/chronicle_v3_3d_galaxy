...
| #   | 问题                                       | 分类    | 分析                                                                                                                                                              | 设计方案关联     |
| --- | ------------------------------------------ | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| T1  | **相机朝向与 Z 轴不平行（应严格平行）**    | ThreeJS | 用户手动测试证实：相机**位置移动方向**沿 Z 轴正常（wheel 仅改 `camera.position.z`），但**视线方向**肉眼并非 Z 平行，需叠加 `yaw≈-15°, pitch≈-7.5°` 才视觉平行。**Rev 3 锁定根因为 Windows 系统缩放 / DPR > 1 兼容性问题**（100% 不复现、125%/150% 稳定复现，右下视窗裁切）——`scene.ts:280-284` 的 `renderer.setSize/setPixelRatio/composer.setSize` 顺序与遗漏 `composer.setPixelRatio` 在 DPR>1 时导致 canvas drawing buffer、viewport、composer RT 三者不一致。修复方向见 §11.1 H-G 与 plan 5.1.4.7 | → **DPR 兼容性修复（plan 5.1.4.7）** |
...
| **T1**  | 相机朝向与 Z 轴不平行（Rev 3 锁定为 DPR>1 兼容性）| 用户测试            | ThreeJS     | 🔴 High   | H-G (5.1.4.7) |
...
> - **Rev 3（2026-04-17 后复测）关键增量**: 现象**仅在 Windows 系统缩放 > 100% 时出现**（100% 不复现、125% / 150% 稳定复现，缩放越高裁切越显著）→ 根因锁定为渲染管线对 `devicePixelRatio > 1` 的兼容性问题，与几何 / 数据层假设（H-A/H-B）无关

**根因调查结论（Rev 3 更新）**:

| 假设 | 状态 | 说明 |
| --- | --- | --- |
| H-E · 运行时 `camera.rotation` 篡改 | ✅ 已排除 | 5.1.4.1 完成；每帧 assert 从未触发 |
| H-D · 投影矩阵 / aspect 异常 | ✅ 已排除（100% 缩放下） | 5.1.4.2 完成；但**仅在 DPR=1 环境验证**，DPR>1 场景下 `aspect` 与 drawing buffer 比例关系需由 H-G 统一处理 |
| H-F · Vertex shader 非预期变换 | ✅ 已排除 | 5.1.4.3 完成；所有 vertex shader 使用标准 MVP |
| H-B · XY 与密度中心偏离 | ✅ 已排除 | 5.1.4.4 完成；改用中位数后偏差依旧，方案 3 不必实施 |
| **H-G · Windows 系统缩放（DPR > 1）兼容性** | 🎯 **已锁定根因** | 100% 不复现 / 125%-150% 复现的可控实验锁定；修复实施见 plan 5.1.4.7 |
| H-A · UMAP 坐标系主轴错配 | ❌ 已取消 | 被 H-G 超越；若修复后仍有残留再复启 |
| H-C · HUD 非对称裁切 | ❌ 已取消 | 被 H-G 超越；若主观构图仍偏可并入 5.3.2 Drawer 打磨 |

**H-G 根因细节**（新增，替代原 6 条假设清单的剩余部分）:

- `frontend/src/three/scene.ts:280-284` 的 `resize()` 逻辑存在三个 DPR 反模式：① `renderer.setSize(w, h, false)` 关闭了 Three.js 对 canvas CSS 尺寸的维护；② `setPixelRatio` 在 `setSize` **之后**调用（挂载期 / resize 首帧存在 DPR 错位过渡态）；③ **从未调用 `composer.setPixelRatio(...)`**，`EffectComposer` 内部 RT 按 CSS 尺寸分配、renderer drawing buffer 按 DPR 缩放尺寸分配，二者比例错位。
- 三者在 DPR=1 时均"表现正常"（各环节的 `_pixelRatio` 都是 1 所以一致），DPR>1 时联合导致 canvas 右下区域不在 WebGL viewport 或 composer 最终 pass 的覆盖范围内——用户主观上解读为"主轴向右下偏、相机朝向不对"。
- 修复方向与验收标准见 plan 5.1.4.7（`scene.ts` resize 重写：`setPixelRatio` 先于 `setSize`、`composer.setPixelRatio` 同步、`updateStyle=true`、DPR 变化监听）。

**修复原则**:

- 不直接把用户实测的 `-15°/-7.5°` 偏移写进 `GALAXY_CAMERA_EULER` — 那只是掩盖症状。
- 保持相机严格 `Euler(0, π, 0, 'YXZ')` 不动，与 Design Spec §2.1「相机轴线始终与 Z 轴平行」一致。
- 本次修复**不**触及 Python 数据管线与 UMAP 产物；设计方案 3（XY 内容中点）保持 §10 的"候选增强"定位，不因 T1 修复而启用。

**历史假设清单（Rev 2 版本，仅供追溯）**:

<details>
<summary>展开查看 Rev 2 时列出的 6 条候选根因假设</summary>

1. **H-A · 坐标系 / 数据系朝向错配** — UMAP 输出主轴未必与世界 +X/+Y 对齐，数据云整体"歪"导致透视下星系主轴倾斜。验证：PCA 主轴分析 / Ortho 视图复测。
2. **H-B · 初始相机 XY 与数据密度中心偏离 + 透视效应** — 相机严格 +Z 看但数据密度偏心导致远方内容飘向一侧。验证：临时用中位数替换 min/max 中点。
3. **H-C · Canvas / HUD 非对称裁切导致的构图中心偏移** — HUD Timeline/Drawer 让用户主观画面中心偏离渲染器几何中心。验证：隐藏 HUD 纯 canvas 下复测。
4. **H-D · 投影矩阵 / aspect 设置异常** — `aspect` 非预期，灭点偏移。验证：打印 `renderer.getSize()` 与 `projectionMatrix` 对照。
5. **H-E · 存在未识别的 `camera.rotation` 副作用写入** — 第三方库 / EffectComposer 内部篡改。验证：tick 循环开头 assert。
6. **H-F · 粒子/球体 shader 内部对 world-space 做非预期变换** — vertex shader 自定义 mat4 作用于 position。验证：审计所有 vertex shader。

原建议排查顺序：H-E → H-D → H-F → H-B → H-A → H-C。实际在 H-B 排除后，用户发现了"DPR 相关"的新线索，路径直接跳转到 H-G。
